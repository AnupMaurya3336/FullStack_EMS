...
const EmployeeCard = ({employee ,onDelete,onEdit}) => {
  ...
  return (
    <div className="group relative card card-hover overflow-hidden" >
        ...
        <div className="relative aspect-5/3 w-full overfull-hidden
            bg-linear-to-br from-slate-200 to-slate-100">
            
            <div className='w-full h-full flex items-center justify-center'>
                {/*circle icon*/}
                <div className="w-18 h-18 rounded-full bg-linear-to-br
                from-indigo-200 to-slate-200 flex items-center justify-center
                transition-all duration-300 hover:scale-120 hover:shadow-lg">
        ...
        <div className='absolute top-3 left-3 flex gap-2'>
            ...
            text-xs front-semibold text-slate-600 rounded-lg shadow-sm'>
        ...
        {!employee.isDeleted && (
            <div className='absolute inset-0 bg-linear-to-t
                ...
                <button  onClick={() => onEdit(employee)} className='p-2.5
                    bg-white/90 backdrop-blur-sm text-slate-700
                    hover:text-indigo-600 rounded-xl shadow-lg transition-all
                    hover:scale-105'>
                    <PencilIcon className='w-4 h-4'/>
                </button>
                <button  onClick={handleDelete}
                    className="p-2.5 bg-white/90 backdrop-blur-sm text-slate-700
                    hover:text-rose-600 rounded-xl shadow-lg transition-all
