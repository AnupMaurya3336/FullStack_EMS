    ...
    <div className='animate-fade-in'>
      ...
      sm:items-center gap-4 mb-8'>
        <div>
          ...
          <p className='page-subtitle'>{isAdmin ? "Manage leave applications" :
            "Your leave history and requests"}</p>
        </div>
        {!isAdmin && !isDeleted && (
          <button onClick={() => setShowModel(true)} className='btn-primary flex
          ...
          {leaveStats.map((s) => (
            <div key={s.label} className='card card-hover p-5 sm:p-6 flex items-center 
              ...
              <div>
                ...
                tracking-tight'>{s.value} <span className='text-sm 
      ...
      <ApplyLeaveModal open={showModel} onClose={()=> setShowModel
        (false)} onSuccess={fetchLeaves}/>
    </div>
  )
}
