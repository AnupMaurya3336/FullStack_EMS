...
const PrintPayslip = () => {
  ...
  useEffect(() => {
    setPayslip(dummyPayslipData.find((slip) => slip._id === id || slip.id === id))
    setTimeout(() => {
      setLoading(false)
    }, 1000)
  }, [id])
  ...
  if (!payslip) return <p className='text-center py-12 
  text-slate-400'>Payslip not found</p>
      ...
      <div className='grid grid-cols-2 gap-6 mb-8'>
        <div>
          ...
          <p className='font-semibold text-slate-900'>
            {payslip.employee?.firstName}{payslip.employee?.lastName}</p>
        </div>
        <div>
          <p className='text-xs text-slate-400 uppercase tracking-wider 
          mb-1'>Position</p>
        ...
        <table className='w-full text-sm'>
          ...
          <tbody>
            <tr className='border-t border-slate-100'>
              ...
              font-medium'>${payslip.basicSalary?.toLocaleString()}</td>
            </tr>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Allowances</td>
              <td className='text-right py-3 px-4 text-slate-900 
              font-medium'>+${payslip.allowances?.toLocaleString()}</td>
            </tr>
            <tr className='border-t border-slate-100'>
              <td className='py-3 px-4 text-slate-700'>Deductions</td>
              <td className='text-right py-3 px-4 text-slate-900 
              font-medium'>-${payslip.deductions?.toLocaleString()}</td>
            </tr>
            <tr className='border-t-2 border-slate-100 bg-slate-50'>
              ...
              text-lg'>${payslip.netSalary?.toLocaleString()}</td>
            </tr>
          </tbody>
        </table>
      </div>
